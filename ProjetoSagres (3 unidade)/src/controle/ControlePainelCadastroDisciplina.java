...
public class ControlePainelCadastroDisciplina implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == telaCadDisciplina.getButtonAdicionarAreaDisciplina()) {
			try {
				areaDisciplina = JOptionPane.showInputDialog(telaCadDisciplina, "Insira o nome da �rea:", "Inserir",
				...
				if (!areaDisciplina.equals("")) {
					...
					if (Validacoes(areaDisciplina, areasDisciplina) == 0) {
						...
						JOptionPane.showMessageDialog(telaCadDisciplina, "Nome j� incluso.", "Erro",
		...
		if (e.getSource() == telaCadDisciplina.getComboBoxCurso()) {
			try {
				...
				for (int i = 0; i < Main.curso.size(); i++) {
					if (telaCadDisciplina.getComboBoxCurso().getSelectedItem().equals(Main.curso.get(i).getNome())) {
						if (Main.curso.get(i).getTipo().equals("MATUTINO")
								|| Main.curso.get(i).getTipo().equals("VESPERTINO")
								|| Main.curso.get(i).getTipo().equals("NOTURNO")) {
							telaCadDisciplina.getComboBoxCargaHoraria().removeAllItems();
							telaCadDisciplina.getComboBoxCargaHoraria().addItem("SELECIONE");
							telaCadDisciplina.getComboBoxCargaHoraria().addItem("30");
							telaCadDisciplina.getComboBoxCargaHoraria().addItem("45");
							telaCadDisciplina.getComboBoxCargaHoraria().addItem("60");
							telaCadDisciplina.getComboBoxCargaHoraria().addItem("75");
							telaCadDisciplina.getComboBoxCargaHoraria().addItem("90");
						} else {
							telaCadDisciplina.getComboBoxCargaHoraria().removeAllItems();
							telaCadDisciplina.getComboBoxCargaHoraria().addItem("SELECIONE");
							telaCadDisciplina.getComboBoxCargaHoraria().addItem("30");
							...
							telaCadDisciplina.getComboBoxCargaHoraria().addItem("75");
							telaCadDisciplina.getComboBoxCargaHoraria().addItem("90");
							telaCadDisciplina.getComboBoxCargaHoraria().addItem("105");
							telaCadDisciplina.getComboBoxCargaHoraria().addItem("120");
							telaCadDisciplina.getComboBoxCargaHoraria().addItem("135");
							telaCadDisciplina.getComboBoxCargaHoraria().addItem("150");
						}
					}
				}
			} catch (Exception e1) {
		...
		if (e.getSource() == telaCadDisciplina.getButtonAdicionarPreRequisitos()) {
			try {
				preRequisito = JOptionPane.showInputDialog(telaCadDisciplina, "Insira o pr�-requisito:", "Inserir",
						JOptionPane.OK_CANCEL_OPTION);
				if (!preRequisito.isEmpty()) {
					preRequisito = preRequisito.toUpperCase();
					int consulta = Integer.parseInt(
							new BancoDisciplinaGerenciar().consultar("nomeDisciplina", preRequisito, "Curso_idCurso"));
					String consulta1 = new BancoCursoGerenciar().consultar("idCurso", String.valueOf(consulta),
							"nomeCurso");
					if (consulta1.equals(telaCadDisciplina.getComboBoxCurso().getSelectedItem().toString())) {
						if (ValidaPreRequisito(preRequisito,
								telaCadDisciplina.getComboBoxCurso().getSelectedItem().toString()) == 1) {
						...
						JOptionPane.showMessageDialog(telaCadDisciplina,
								"Disciplina cadastrada em outro curso, n�o no curso escolhido. "
		...
		// try {
		// semestreInserido =
		// Integer.parseInt(JOptionPane.showInputDialog(telaCadDisciplina, "Insira o
		// semestre:",
		...
		// }
		// }catch (Exception e1) {
		// JOptionPane.showMessageDialog(telaCadDisciplina, "Valor inserido n�o
		// aceito.",
		// "Erro", JOptionPane.ERROR_MESSAGE);
		// System.out.println(e1.getMessage());
		// e1.printStackTrace();
		...
		if (e.getSource() == telaCadDisciplina.getButtonConfirmar()) {
			...
			try {
				...
				if (!telaCadDisciplina.getComboBoxAreaDisciplina().getSelectedItem().equals("SELECIONE")) {
					...
					JOptionPane.showMessageDialog(telaCadDisciplina, "Campo �rea n�o selecionado!", "Aviso",
							JOptionPane.ERROR_MESSAGE);
					erros++;
				}
				if (!telaCadDisciplina.getComboBoxCargaHoraria().getSelectedItem().equals("SELECIONE")) {
					...
					JOptionPane.showMessageDialog(telaCadDisciplina, "Campo Carga Hor�ria n�o selecionado!", "Aviso",
							JOptionPane.ERROR_MESSAGE);
					erros++;
				}
				if (!telaCadDisciplina.getComboBoxCurso().getSelectedItem().equals("SELECIONE")) {
					...
					JOptionPane.showMessageDialog(telaCadDisciplina, "Campo Curso n�o selecionado!", "Aviso",
							JOptionPane.ERROR_MESSAGE);
					erros++;
				}
				if (!telaCadDisciplina.getComboBoxSemestre().getSelectedItem().equals("SELECIONE")) {
					...
					JOptionPane.showMessageDialog(telaCadDisciplina, "Campo Semestre n�o selecionado!", "Aviso",
							JOptionPane.ERROR_MESSAGE);
					erros++;
				}
				if (!telaCadDisciplina.getFormattedTextFieldNomeDisciplina().getText().equals("")) {
					...
					if (!telaCadDisciplina.getComboBoxCurso().getSelectedItem().equals("SELECIONE")) {
						...
						if (ValidaNomeDisciplina(nome_Disciplina, nomeCurso) == 0) {
							...
							JOptionPane.showMessageDialog(telaCadDisciplina,
									"Disciplina com mesmo nome e curso j� cadastrados.", "Aviso",
									JOptionPane.ERROR_MESSAGE);
						}
					} else {

						JOptionPane.showMessageDialog(telaCadDisciplina, "Selecione o curso.", "Aviso",
								JOptionPane.ERROR_MESSAGE);

					}

				} else {
					JOptionPane.showMessageDialog(telaCadDisciplina, "Campo Nome n�o preenchido!", "Aviso",
							JOptionPane.ERROR_MESSAGE);
					erros++;
				}
				if (telaCadDisciplina.getRadioButtonOptativa().isSelected()) {
					...
					JOptionPane.showMessageDialog(telaCadDisciplina,
							"Indique se a disciplina � optativa ou obrigat�ria.", "Aviso", JOptionPane.ERROR_MESSAGE);
					erros++;
				}
				...
				if (contador == 6) {
					...
					if (preRequisitos.size() > 0) {
						int idDisciplina = Integer
								.parseInt(new Banco().primeiroEultimo("Disciplina", "idDisciplina", 2));
						for (int i = 0; i < preRequisitos.size(); i++) {

							try {
								int idPreRequisito = Integer.parseInt(new BancoDisciplinaGerenciar()
										.consultar("nomeDisciplina", preRequisitos.get(i).toString(), "idDisciplina"));
								retorno1 = new BancoDisciplinaGerenciar().inserirPreRequisitos(idDisciplina,
										idPreRequisito, preRequisitos.get(i));
					...
					if (contador != 6)
						...
						JOptionPane.showMessageDialog(telaCadDisciplina, "ERRO DURANTE O CADASTRO.", "Aviso",
	...
	public int ValidaNomeDisciplina(String nome, String curso) {
		int contador = 0;
		ArrayList<String> disciplinas = new BancoDisciplinaGerenciar().consultarUmaColuna("Disciplina",
				"nomeDisciplina");
		for (int i = 0; i < disciplinas.size(); i++) {
			if (disciplinas.get(i).equals(nome)) {
				// selecionando o id do curso em que a disciplina est� cadastrada
				int idCursoDisciplina = Integer
						.parseInt(new Banco().consultar("Disciplina", "nomeDisciplina", nome, "Curso_idCurso"));
				int idCurso = Integer.parseInt(new BancoCursoGerenciar().consultar("nomeCurso", curso, "idCurso"));
				if (idCursoDisciplina == idCurso)
	...
	public void preenche() {
		...
		try {
			...
			ArrayList<String> disciplinasArea = new BancoDisciplinaGerenciar().consultarUmaColuna("Disciplina",
					"areaDisciplina");
