...
public class ControlePainelCadastroClasse implements ActionListener{
	...
	public ControlePainelCadastroClasse(VisaoFramePrincipal framePrincipal, VisaoPainelCadastroClasse telaCadClasse, int volta) {
		this.telaCadClasse = telaCadClasse;
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == telaCadClasse.getButtonVoltar()) {
			try {
				saida = JOptionPane.showConfirmDialog(telaCadClasse, 
														"Deseja realmente voltar o programa?",
		...
		if(e.getSource() == telaCadClasse.getButtonAdicionarAluno()) {
			if(!telaCadClasse.getComboBoxDisciplina().getSelectedItem().equals("SELECIONE")) {
					...
					if(!String.valueOf(matricula).equals("")) {
						if(ValidaAluno(matricula, 1) == 1) {
							if(ValidaAluno(matricula, 2) == 0) {
								...
								JOptionPane.showMessageDialog(null, "Aluno não cumpriu o(s) pré-requisito(s) da disciplina.", "Aviso", JOptionPane.ERROR_MESSAGE); 
								
							}
						}else {
							 JOptionPane.showMessageDialog(null, "Número de matrícula não encontrado no banco de dados.", "Erro", JOptionPane.ERROR_MESSAGE); 
						}
				...
				JOptionPane.showMessageDialog(telaCadClasse,  "Insira o nome da disciplina", 
				...
				try {
					...
					if(!String.valueOf(matricula).equals("")) {
						if(ValidaProfessor(matricula, 1) == 1) {
							...
							JOptionPane.showMessageDialog(telaCadClasse, "Número de matrícula não encontrado.", 
					...
					System.out.println(e1.getMessage());
					JOptionPane.showMessageDialog(telaCadClasse, "Erro durante a busca.", 
							"Erro", JOptionPane.ERROR_MESSAGE);
					e1.printStackTrace();
				}
		}
		...
		if(e.getSource() == telaCadClasse.getButtonGerarHorario()) {
			...
			if(visaoHorarios == null) {
				visaoHorarios = new VisaoPainelHorarios(telaCadClasse.getComboBoxCurso().getSelectedItem().toString(), tipoCurso);
			}
			framePrincipal.trocarPainel(visaoHorarios, "Horários");
			new ControlePainelHorários(framePrincipal, visaoHorarios, tipoCurso, 
	...
	public int ValidaProfessor(String numero, int tipo) {
		...
		// 3 - verifica se o professor já está cadastrado em outra turma no mesmo horário e dia
		...
		if(tipo == 1) {
			...
			for(int i = 0; i < Main.professor.size(); i++) {
				if( Main.professor.get(i).getMatricula().equals(numero)) {
					if(!telaCadClasse.getComboBoxDisciplina().getSelectedItem().equals("SELECIONE")) {
						int j = 0;
						while(!telaCadClasse.getComboBoxDisciplina().getSelectedItem().equals(
	...
	public int ValidaAluno(String numero, int tipo) {
		...
		// 2 - verifica se o aluno está cadastrado em outra disciplina no mesmo horário e dia
				...
				if(Main.aluno.get(i).getMatricula().equals(numero)) {
					...
					for(int j = 0; j < tamanho; j++) {
						for(int k = 0; k < Main.aluno.get(i).getHistoricoDisciplinas().size(); k++) {
							if(Main.aluno.get(i).getHistoricoDisciplinas().get(k).equals(
									Main.disciplina.get(index).getPreRequisitos().get(j))) {
								contador++;
							}
						}
					}
	...
	public void preencheComboBox(int tipo) {
		...
		// 2 - preenche disciplina --- insere no combobox disciplinas cadastradas em cursos
	...
	public String busca() {
		// retorna uma string que indica qual o tipo do curso (matutino, vespertino, noturno ou integral)
